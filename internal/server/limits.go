...
func DefaultLimits() Limits {
	...
	claims := validateLicenseKey(key, "recipe")
	if claims == nil {
	...
	if claims.Tier != "individual" && claims.Tier != "*" {
		...
		for _, t := range claims.Tools {
			if t == "recipe" || t == "*" { found = true; break }
		}
		if !found {
			log.Printf("[license] Tool recipe not in licensed tools")
			return NoLicense()
		}
	}
...
func (s *Server) tierHandler(w http.ResponseWriter, r *http.Request) {
	...
	m["upgrade_url"] = "https://stockyard.dev/recipe/"
	wj(w, 200, m)
}
